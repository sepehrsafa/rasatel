...
class MessageBoxSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MessageBox
        fields = ["id", "contact", "messages", "number", "read"]
...
class SendMessage(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        msgBox, created = MessageBox.objects.get_or_create(
            user=user,
            goipPort=goipPort,
        ...
        msg = Message.objects.create(
            messageBox=msgBox, message=validated_data["message"], messageType=0
        )
    ...
    def validate(self, validated_data):
        ...
        port = validated_data["port"]
        try:
            validated_data["contact"] = Contact.objects.get(
                belongsTo=user, number=validated_data["number"]
            )
        ...
        try:
            ...
            raise serializers.ValidationError(
                "the user does NOT have access to the port"
            )
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            msgBox, created = MessageBox.objects.get_or_create(
                user=goipPort.user,
                goipPort=goipPort,
                number=validated_data["from_number"],
            )
            if not created:
                msgBox.read = False
                msgBox.save()
            msg = Message.objects.create(
                messageBox=msgBox, message=validated_data["content"], messageType=1
            )
            return msg
